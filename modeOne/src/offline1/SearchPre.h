...
struct candidate{
    ...
    bool operator<(const candidate & other)const{
        if(this->distan != other.distan){
            return  this->distan < other.distan;
        }else if(this->word.size() != other.word.size()){
            return this->word.size() > other.word.size();
        }
...
public:
    ...
    set<struct candidate> generateAndSortCandidate(const string& enterQuery);
