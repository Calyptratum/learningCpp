...
vector<struct candidate> SearchPre::generateAndSortCandidate( const string & enterQuery){
    set<int> resultSet;
    ...
    std::vector<struct candidate> recommend;
    for(int idx :resultSet){
        if(idx >= 0 && idx <(int)_dict.size()){
            ...
            recommend.emplace_back(tmp);
        }
    }
    sort(recommend.begin(),recommend.end(),
         [](
            const struct candidate & a,
            const struct candidate & b
           ){
